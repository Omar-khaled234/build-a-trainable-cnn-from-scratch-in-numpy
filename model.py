...
def stable_softmax(logits):
    ...
    exp_shift = exp_shifted(logits)
    sum_of_rows = row_sum(exp_shift)
    return exp_shift / sum_of_rows
...
def he_init(shape, fan_in, seed):
    ...
    np.random.seed(seed)
    weights = np.random.normal(0 , he_std(fan_in), size=shape)
    return weights
...
def pad_2d(images, pad):
    ...
    new_h = H + 2*pad
    new_w = W + 2*pad
    padded = np.zeros((N, C, new_h, new_w), dtype=images.dtype)
